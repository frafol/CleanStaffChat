...
public class ChatListener extends ListenerAdapter {
    ...
    public void onChat(PlayerChatEvent event) {
        ...
        if (ADMINCHAT_PREFIX_MODULE.get(Boolean.class) && event.getMessage().startsWith(ADMINCHAT_PREFIX.get(String.class))) {
            final String message = event.getMessage().substring(1);
            if (!event.getPlayer().hasPermission(ADMINCHAT_USE_PERMISSION.get(String.class))) return;
