...
public class ChatListener extends ListenerAdapter implements Listener {
    ...
    public void onChat(@NotNull ChatEvent event) {
        ...
        if (BungeeConfig.STAFFCHAT_PREFIX_MODULE.get(Boolean.class) && event.getMessage().startsWith(BungeeConfig.STAFFCHAT_PREFIX.get(String.class))) {
            final String message = event.getMessage().substring(1);
            if (!((ProxiedPlayer) event.getSender()).hasPermission(BungeeConfig.STAFFCHAT_USE_PERMISSION.get(String.class))) return;
