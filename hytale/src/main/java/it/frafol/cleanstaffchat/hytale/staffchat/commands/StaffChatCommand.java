...
public class StaffChatCommand extends AbstractCommand {
    ...
    public StaffChatCommand(CleanStaffChat plugin, String name, String description, java.util.List<String> aliases) {
        super(name, description);
        this.plugin = plugin;
        this.setAllowsExtraArguments(true);
        this.requirePermission(Objects.requireNonNull(HytaleConfig.STAFFCHAT_USE_PERMISSION.get(String.class)));
        if (aliases != null) {
    ...
    protected CompletableFuture<Void> execute(@Nonnull CommandContext context) {
        ...
        if (messageArg.isEmpty()) {
            ...
            if (senderHasPerm) {
                if (!PlayerCache.getToggled_2().contains(sender.getUuid())) {
                    if (!Boolean.TRUE.equals(HytaleConfig.STAFFCHAT_TALK_MODULE.get(Boolean.class))) {
                        sender.sendMessage(HytaleMessages.ARGUMENTS.color());
                        return CompletableFuture.completedFuture(null);
                    }

                    if (!PlayerCache.getMuted().contains("true")) {
                        PlayerCache.getToggled_2().add(sender.getUuid());
                        sender.sendMessage(HytaleMessages.STAFFCHAT_TALK_ENABLED.color());
                    }
                } else {
                    PlayerCache.getToggled_2().remove(sender.getUuid());
                    sender.sendMessage(HytaleMessages.STAFFCHAT_TALK_DISABLED.color());
                }
            } else {
        ...
        if (PlayerCache.getMuted().contains("true")) {
            sender.sendMessage(HytaleMessages.STAFFCHAT_MUTED_ERROR.color());
            return CompletableFuture.completedFuture(null);
        }

        String senderName = (sender instanceof Player) ? sender.getDisplayName() : HytaleConfig.CONSOLE_PREFIX.get(String.class);
        String rawFormat = HytaleMessages.STAFFCHAT_FORMAT.get(String.class);
    ...
    private void sendToDiscord(String user, String message) {
        if (Boolean.TRUE.equals(HytaleDiscordConfig.DISCORD_ENABLED.get(Boolean.class))
            ...
            String discordFormat = HytaleMessages.STAFFCHAT_FORMAT_DISCORD.get(String.class)
                    .replace("{user}", user)
