...
public class ToggleCommand extends AbstractCommand {
    ...
    public ToggleCommand(CleanStaffChat plugin, String name, String description, List<String> aliases) {
        ...
        this.requirePermission(Objects.requireNonNull(HytaleConfig.DONORCHAT_TOGGLE_PERMISSION.get(String.class)));
        if (aliases != null) {
    ...
    protected CompletableFuture<Void> execute(@Nonnull CommandContext context) {
        ...
        if (!Boolean.TRUE.equals(HytaleConfig.DONORCHAT_TOGGLE_MODULE.get(Boolean.class))) {
        ...
        if (!PermissionsUtil.hasPermission(sender.getUuid(), HytaleConfig.DONORCHAT_TOGGLE_PERMISSION.get(String.class))) {
        ...
        if (!PlayerCache.getToggled_donor().contains(sender.getUuid())) {
            PlayerCache.getToggled_donor().add(sender.getUuid());
            String toggledOff = HytaleMessages.DONORCHAT_TOGGLED_OFF.get(String.class)
                    .replace("{prefix}", prefix != null ? prefix : "");
            sender.sendMessage(ChatColor.color((toggledOff)));
        } else {
            PlayerCache.getToggled_donor().remove(sender.getUuid());
            String toggledOn = HytaleMessages.DONORCHAT_TOGGLED_ON.get(String.class)
                    .replace("{prefix}", prefix != null ? prefix : "");
